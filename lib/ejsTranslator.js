(function (ejsTranslator) {
        ...
        const TRANSACTIONAL_TMP_DELIMITER = "____";
        ...
        function extractOperator(clause) {
            var foundOperator = "";
            var operatorsInClause = consts.LEGAL_OPERATORS.filter(function (operator) {
                var operatorIndex = clause.indexOf(operator);
            ...
            switch (operatorsInClause.length) {
                ...
                case 1:
                    break;
                case 2:
                    correctList1 = [consts.OP_LT, consts.OP_LTE].sort();
                    correctList2 = [consts.OP_GT, consts.OP_GTE].sort();
                    ...
                    if ((_.isEqual(operatorsInClause, correctList1)) || (_.isEqual(operatorsInClause, correctList2))) {
                        var longestOperator = operatorsInClause.sort(function (a, b) {
            ...
            if (operatorsInClause.length >= 1) {
                if (operatorsInClause.length < 3 && consts.LEGAL_OPERATORS.indexOf(operatorsInClause[0]) >= 0) {
                    foundOperator = operatorsInClause[0];
                }
                else {
                    addValidationResult(clause, consts.VALIDATION_ILLEGAL_OP_MSG, `Operators found: ${operatorsInClause.join(',')}`);
                }
            }
            return foundOperator;
        }

        function encloseAttr(attr) {
            return `${consts.ATTRIBUTES_KEY_STR}['${attr}']`;
        }

        function decloseAttr(attr) {
            if (attr) {
                attr = attr.replace(`${consts.ATTRIBUTES_KEY_STR}['`, "").replace("']", "");
            }
            return attr;
        }
        ...
        function extractClauseParts(clause) {
            var clausePartsObj = {
                ...
                [consts.CUR_OP_KEY]: extractOperator(clause),
                [consts.CUR_VAL_KEY]: ""
            };
            if (clause && clausePartsObj[consts.CUR_OP_KEY].trim()!="") {
                ...
                clausePartsObj[consts.CUR_VAL_KEY] = clause.substring(clause.indexOf(clausePartsObj[consts.CUR_OP_KEY])+clausePartsObj[consts.CUR_OP_KEY].length);
            }
            //todo: check if clausePartsObj[consts.CUR_VAR_KEY] is valid (i.e. a legal attribute - maybe smthing like if(!(clausePartsObj[consts.CUR_VAR_KEY] in attrs)))
        ...
        ejsTranslator.translateToEjs = function (dataToTranslate, attrsObj, shouldReturnGroupByMapping, shouldValidate, maxClausesPerBlock, maxCondBlocks) {
            ...
            */
            ...
            function updateGroupByVars(variable, operator = consts.EXISTENCE_OPERATOR_FOR_GROUPBY,
                                       val = null) {
                if (!GroupByVars[variable]) {
                    GroupByVars[variable] = {};
                }
                if (!GroupByVars[variable][operator]) {
                    GroupByVars[variable][operator] = new Set();
                }
                if (operator == consts.EXISTENCE_OPERATOR_FOR_GROUPBY) {
                    return;
                }
            ...
            function getClauseToInsertObj(clauseStr, openDelimiter = consts.CUSTOM_OPEN_DELIMITER, closeDelimiter = consts.CUSTOM_CLOSE_DELIMITER) {
                ...
                clauseToInsert = (conditionStr.endsWith("IF")) ?
                    clauseToInsert.substring(delimiterIndex + consts.LOGICAL_CONDITION_DELIMITER.length)
                        .trim().toUpperCase() : "";
                if (clauseToInsert != "") {
                    //res[consts.CUR_CLAUSE_KEY] = `${consts.ATTRIBUTES_KEY_STR}.${clauseToInsert}`;
                    var clauseParts = extractClauseParts(clauseToInsert);
                    res[consts.CUR_CLAUSE_KEY] = `${encloseAttr(clauseParts[consts.CUR_VAR_KEY])}${clauseParts[consts.CUR_OP_KEY]}${clauseParts[consts.CUR_VAL_KEY]}`;
                }
                ...
                if (conditionStr.startsWith("ELSE")) {
                    ...
                    var mappedClauses = Array.from(ClausesSet).map(x => `!(${x})`);
                    //var negationOfPastConditions = `!((${mappedClauses.join(")||(")}))`;
                    var negationOfPastConditions = `(${mappedClauses.join("&&")})`;
                    /*if (ClausesSet.size == 1) {
                        negationOfPastConditions = negationOfPastConditions.replace("!((", "!(").replace("))", ")");
                    }*/
            ...
            function formatAttr(attr) {
                ...
                if (shouldFormat) {
                    ...
                    tagPrefixes.PersonalizationTypesPrefixes.forEach(function (prefix) {
                        var prefixStartIndex = attr.indexOf(`${prefix}${consts.LOGICAL_CONDITION_DELIMITER}`);
                        if (prefixStartIndex >= 0) {
                            var origSubstring = attr.substring(prefixStartIndex);
                            var delimRegex = new RegExp(`${consts.LOGICAL_CONDITION_DELIMITER}`, 'g');
                            var replacedSubstring = origSubstring.replace(delimRegex, `${TRANSACTIONAL_TMP_DELIMITER}`);
                            attr = attr.replace(origSubstring, replacedSubstring);
                        }
                    });
                }
                return attr;
