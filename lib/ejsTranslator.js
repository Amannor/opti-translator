(function (ejsTranslator) {
    ...
    function getValidationErrors(strToValidate)
    {
        var validationItem1 = {};
        validationItem1.Condition = "[%IF::EMAIL!='opti@optimove.com'%]";
        validationItem1.Error = "Invalid condition";
        validationItem1.LineNum = 30;
        validationItem1.details = "Exception caught at..";
        var validationItem2 = {};
        validationItem2.Condition = "[%ELSEIF::EMAIL!='optimi@optimove.com'%]";
        validationItem2.Error = "Invalid condition";
        validationItem2.LineNum = 40;
        validationItem2.details = "Exception caught at..";
        var items = [validationItem1, validationItem2];
        return items;
    }

    ejsTranslator.translateToEjs = function (dataToTranslate, attrsObj, shouldReturnGroupByMapping, shouldValidate ) {
        var validationErrors = [];
        if(shouldValidate)
        {
           validationErrors = getValidationErrors(dataToTranslate);
        }

        //comment to tst source control
        ...
        */
        ...
        function getClauseToInsertObj(clauseStr, openDelimiter = consts.CUSTOM_OPEN_DELIMITER, closeDelimiter = consts.CUSTOM_CLOSE_DELIMITER) {
            ...
            var clauseToInsert = clauseStr.trim()
        ...
        function attrsToUpperCase(attrsObjToCast) {
            ...
            Object.keys(attrsObjToCast).forEach(function (attr) {
                ...
                attrsObjToCast[attr.replace('[%', '').replace('%]', '').toUpperCase()] = attrsToUpperCase(tmp);
            });
            return attrsObjToCast;
        } 
        ...
        while (openingClauseStartIndex >= 0 && strLeft) {
            ...
            strLeft = strLeft.substring(endClauseStartIndex + consts.CUSTOM_CLOSING_IF_BLOCK_TAG.length);
            openingClauseStartIndex = getNextClauseIndex(strLeft);
        }
        var resStr = "";
        if (attrsObj) {
            attrsObj = attrsToUpperCase(attrsObj);
        ...
        var outputObj = {
            ...
            [consts.VALIDATE_OUTPUT_KEY_STR]: validationErrors
        };
        return outputObj;
    };
