...
class condFormatter {
  ...
  trimStr(fullStr, strToStrip = consts.STRING_AND_DATE_LITERAL_ENCLOSING) {
    const first = [...fullStr].findIndex(char => char !== strToStrip);
    const last = [...fullStr].reverse().findIndex(char => char !== strToStrip);
    return fullStr.substring(first, fullStr.length - last);
  }
  ...
  isLegalValue(valueStr) {
    valueStr = valueStr.trim();
    if ('0123456789'.indexOf(valueStr.charAt(0)) >= 0) {
      return true;
    }
    if (valueStr.charAt(0) === consts.STRING_AND_DATE_LITERAL_ENCLOSING && valueStr.slice(-1) === consts.STRING_AND_DATE_LITERAL_ENCLOSING) {
      return true;
    }
    return false;
  }
  ...
  getFormattedClauseToInsert(clause) {
    ...
    if (clause && consts.LEGAL_OPERATORS.indexOf(clausePartsObj[consts.CUR_OP_KEY]) >= 0) {
      ...
      if (!(this.isLegalValue(clausePartsObj[consts.CUR_VAL_KEY]))) {
        this.reportValidationRes(clause, consts.VALIDATION_ILLEGAL_VALUE_MSG, `Value detected: ${clausePartsObj[consts.CUR_VAL_KEY]}`);
      }
  ...
  extractOperator(clause) {
    let foundOperator = '';
    const clauseToSearchIn = clause.indexOf(consts.STRING_AND_DATE_LITERAL_ENCLOSING) >= 0 ?
      clause.substring(0, clause.indexOf(consts.STRING_AND_DATE_LITERAL_ENCLOSING)) :
      clause;
    let operatorsInClause = consts.LEGAL_OPERATORS.filter((operator) => {
      const operatorIndex = clauseToSearchIn.indexOf(operator);
  ...
  clausePartsToDateTimeClauseStr(clausePartsObj, formatFound) {
    if (!formatFound || formatFound.trim() === '') {
      ...
      formatFound = `${consts.STRING_AND_DATE_LITERAL_ENCLOSING}${formatFound}${consts.STRING_AND_DATE_LITERAL_ENCLOSING}`;
    }
  ...
  isExplicitValidDateTimeFormat(formatToCheck, explicitFormats) {
    ...
    formatToCheck = formatToCheck.replace(new RegExp(datetimeHelper.DATETIME_DELIMITERS.join('|'), 'g'), '');
    formatToCheck = formatToCheck.replace(new RegExp(String.raw`\\`, 'g'), '');
  ...
  tryGetClauseDateTimeFormat(variable, op, value) {
    ...
    if (explicitDateTimeObj.isExplicitFormat === true) {
      ...
      const strippedValue = this.trimStr(value);
      if (moment(strippedValue, momentFormat).isValid()) {
        return { isValidDateTime: true, format: momentFormat };
      }

      if (consts.EGALITARIAN_OPERATORS.indexOf(op) < 0) {
        // We report an error in this case only if we can't perform a regular string comparison
    ...
    const variableMomentFormat = this.varsToValues[variable] ? this.trimStr(momentParseFormat(this.varsToValues[variable])) : '';

    const valueMomentFormat = value ? this.trimStr(momentParseFormat(value)) : '';
...
condFormatter.condDateTimeFormatToMomentFormatMap = {
  ...
  [datetimeHelper.DATE_TYPE_ID]: {
    ...
    // This entry is a workaround in case we manually uppercased the valid format given
    [datetimeHelper.dateFormats.d4.toUpperCase()]: datetimeHelper.dateFormats.d4,
  },
