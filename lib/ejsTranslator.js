  ...
  ejsTranslator.translateToEjs = function (dataToTranslate, attrsObj, shouldReturnGroupByMapping, shouldValidate, maxClausesPerBlock, maxCondBlocks) {
    ...
    if (attrsToEjs) {
      ...
      try {
        ...
        addValidationResult('', `Error in ejs.render.\nlint obj details: ${JSON.stringify(lintRes)}`, `Original Exception details: ${error}`, consts.VALIDATION_UNASSIGNED_INDEX, consts.VALIDATION_UNASSIGNED_INDEX);
      }
    }
    if (shouldReturnGroupByMapping) {
