...
(function (ejsTranslator) {
  ...
  function extractClauseParts(clause) {
    ...
    if (legalAttributes.indexOf(clausePartsObj[consts.CUR_VAR_KEY]) < 0) {
      addValidationResult(
        `${clausePartsObj[consts.CUR_VAR_KEY]}${clausePartsObj[consts.CUR_OP_KEY]}${clausePartsObj[consts.CUR_VAL_KEY]}`,
  ...
  ejsTranslator.translateToEjs = function (dataToTranslate, attrsObj, shouldReturnGroupByMapping, shouldValidate, maxClausesPerBlock, maxCondBlocks) {
    ...
    function updateGroupByVars(
      variable, operator = consts.EXISTENCE_OPERATOR_FOR_GROUPBY,
      val = null,
    ) {
    ...
    function getClauseToInsertObj(clauseStr /* openDelimiter = consts.CUSTOM_OPEN_DELIMITER, closeDelimiter = consts.CUSTOM_CLOSE_DELIMITER */) {
      ...
      let clauseToInsert = formatAttr(clauseStr);
      ...
      clauseToInsert = (conditionStr.endsWith('IF')) ?
        clauseToInsert.substring(delimiterIndex + consts.LOGICAL_CONDITION_DELIMITER.length)
          .trim().toUpperCase() : '';
      if (clauseToInsert !== '') {
        const clauseParts = extractClauseParts(clauseToInsert);
    ...
    function preprocessAttrs(attrsObjToCast) {
      ...
      if (postProcessinhAttrsObj instanceof Number || typeof postProcessinhAttrsObj === 'number') {
        ...
        if (postProcessinhAttrsObj[0] instanceof String || typeof postProcessinhAttrsObj[0] === 'string') {
          postProcessinhAttrsObj = postProcessinhAttrsObj.map(s => formatAttr(s));
        }
        return postProcessinhAttrsObj;
      }
      Object.keys(postProcessinhAttrsObj).forEach((attr) => {
        ...
        if (tmp != null) {
          postProcessinhAttrsObj[formatAttr(attr)] = preprocessAttrs(tmp);
        }
      });
      return postProcessinhAttrsObj;
    }

    let strLeft = decode(dataToTranslate);
    ...
    if (attrsToEjs) {
      ...
      /* todo: this fails in case there's IF:10<=PUBLIC_CUSTOMER_ID because it makes the translation be like:
                            ...
                            */
      try {
        resStr = ejs.render(newStr, { attrs: attrsToEjs });
      } catch (error) {
