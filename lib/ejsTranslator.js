...
(function (ejsTranslator) {
  ...
  function updateCondBlocksCounters(condClause) {
    if (condClause.startsWith(consts.CUSTOM_OPENING_IF_BLOCK_PREFIX)) {
      ...
      if (maxAllowedConditionalBlocks && typeof maxAllowedConditionalBlocks === 'number' && maxAllowedConditionalBlocks > 0 && maxAllowedConditionalBlocks < curCondBlockId + 1) {
        addValidationResult(condClause, `${consts.VALIDATION_BLOCK_MSG}: Too many condition blocks (may include up to ${maxAllowedConditionalBlocks} blocks)`);
      }
      ...
      if (maxAllowedClausesPerBlock && typeof maxAllowedClausesPerBlock === 'number' && maxAllowedClausesPerBlock > 0 && maxAllowedClausesPerBlock < curInnerBlockId + 1) {
        addValidationResult(condClause, `${consts.VALIDATION_BLOCK_MSG}: Too many clauses in block (block may include up to ${maxAllowedClausesPerBlock} clauses)`);
      }
    }
  }
  ...
  function getOpeningClauseEndIndex(strToCheck) {
  ...
  ejsTranslator.translateToEjs = function (dataToTranslate, attrsObj, shouldReturnGroupByMapping, shouldValidate, maxClausesPerBlock, maxCondBlocks) {
    ...
    while (openingClauseStartIndex >= 0 && strLeft) {
      const openingClauseEndIndex = getOpeningClauseEndIndex(strLeft.substring(openingClauseStartIndex))+openingClauseStartIndex;
      /*
          ...
          *Consider deleting the inner [%%] of the varName from the innerString (i.e. between the outermost [%%]) prior to proceeding (so that everything else will remain as is)
                            * */
      if (openingClauseStartIndex > 0) {
        newStr += strLeft.substring(0, openingClauseStartIndex);
      }
      let openingClause = strLeft.substring(openingClauseStartIndex, openingClauseEndIndex).trim();
      ...
      if (openingClause.startsWith(`${consts.CUSTOM_OPEN_DELIMITER}IF${consts.LOGICAL_CONDITION_DELIMITER}`)) {
        ClausesSet = new Set();
      }
    ...
    if (attrsToEjs) {
      ...
      try {
        ...
        // Note: We're pushing to returned validation results regardless of flag 'shouldReturnValidations' - because this error is fundemental
        const { VALIDATION_UNASSIGNED_INDEX } = consts;
        const errMsg = `Error in ejs.render.\nlint obj details: ${JSON.stringify(lintRes)}`;
        const detailsArg = `Original Exception details: ${error}`;

        const res = new ValidationResult(VALIDATION_UNASSIGNED_INDEX, VALIDATION_UNASSIGNED_INDEX, '', errMsg, detailsArg);
        validationResults.push(res);
      }
    }
    if (shouldReturnGroupByMapping) {
