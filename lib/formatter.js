...
class condFormatter {
  ...
  convertToMomentFormat(date) {
    Object.keys(condFormatter.condDateTimeFormatToMomentFormatMap).forEach((condFormat) => {
      date = date.replace(new RegExp(condFormat, 'g'), condFormatter.condDateTimeFormatToMomentFormatMap[condFormat]);
    });
    return date;
  }
  ...
  getFormattedClauseToInsert(clause) {
    ...
    if (clause && consts.LEGAL_OPERATORS.indexOf(clausePartsObj[consts.CUR_OP_KEY]) >= 0) {
      ...
      if (dateTimeResObj.isValidDateTime) {
        const dateTimeClausePartsObj = Object.assign({}, clausePartsObj);
    ...
    for (let i = 0; i < explicitDateTimeObjList.length; i += 1) {
      ...
      for (let j = 0; j < tags.length; j += 1) {
        ...
        if (variable === tag) {
          ...
          let explicitFormatFound = variable.substring(delimiterIndex + consts.LOGICAL_CONDITION_DELIMITER.length);
          if (explicitFormatFound.trim() === curObj[datetimeHelper.DATETIME_DEF_FORMAT_ALIAS_KEY]) {
            explicitFormatFound = curObj[datetimeHelper.DATETIME_DEF_FORMAT_KEY];
          }
          if (this.isExplicitValidDateTimeFormat(explicitFormatFound, curObj[datetimeHelper.DATETIME_TAGS_KEY])) {
  ...
  tryGetClauseDateTimeFormat(variable, op, value) {
    ...
    // If both are false & non-egaliterian operator - addValidation Error
    return { isValidDateTime: false };
  }
}
