...
(function (ejsTranslator) {
        ...
        function formatAttr(attr) {
            ...
            if (shouldFormat) {
                ...
                let prefixesToRemove = tagPrefixes.PersonalizationTagsPrefixes;
        ...
        function init(attrsObj) {
            ...
            try {
                legalAttributes = (typeof attrsObj != 'function' && (typeof attrsObj !== 'object' || attrsObj == null)) ? [] : Object.keys(attrsObj).map(x => formatAttr(x));
            }
            catch (e) {
                legalAttributes = [];
            }
        ...
        function extractClauseParts(clause) {
            ...
            if (legalAttributes.indexOf(clausePartsObj[consts.CUR_VAR_KEY]) < 0) {
                addValidationResult(`${clausePartsObj[consts.CUR_VAR_KEY]}${clausePartsObj[consts.CUR_OP_KEY]}${clausePartsObj[consts.CUR_VAL_KEY]}`,
        ...
        ejsTranslator.translateToEjs = function (dataToTranslate, attrsObj, shouldReturnGroupByMapping, shouldValidate, maxClausesPerBlock, maxCondBlocks) {
            ...
            */
            ...
            var openingClauseStartIndex = getNextClauseIndex(dataToTranslate);
            while (openingClauseStartIndex >= 0 && (!(dataToTranslate.substring(openingClauseStartIndex).startsWith(consts.CUSTOM_OPENING_IF_BLOCK_PREFIX)))) {
                let condClauseIndex = dataToTranslate.substring(openingClauseStartIndex).indexOf(consts.CUSTOM_CLOSE_DELIMITER);
                let condClause = condClauseIndex >= 0 ? dataToTranslate.substring(openingClauseStartIndex, condClauseIndex + consts.CUSTOM_CLOSE_DELIMITER.length) : dataToTranslate.substring(openingClauseStartIndex);
                curCondBlockId++;
